...
Theorem prog : forall M G, typ_sess M G -> 
  (forall M_int, unfoldP M M_int -> exists M', unfoldP M_int M' /\ (ForallT (fun _ P => P = p_inact) M')) 
  \/ exists M', betaP M M'.
Proof.
	intros * Hsess.
  inversion Hsess;subst. 
	...
	{
    left. intros. eapply typ_after_unfold in H;try exact Hsess.
   	eapply canonical_glob_n_strong in Hdil;try exact H. easy.
	}
	{
