...
lemma.projection_helper src.lcontext lemma.projection lemma.subj_red_helpers lemma.subj_red_prog_fid lemma.live_proc lemma.fairness_feasible.
...
Module ParScong.
  (** ** List instances *)
  #[export] Instance aac_par_assoc : Associative scong s_par.
  Proof. constructor. Qed.
  #[export] Instance aac_par_unit  : Unit scong s_par s_zero. Proof.
    constructor;intros; eauto with brocs.
  Qed. 
  ...
  #[export] Instance aac_scong_proper2 : forall x, Proper (scong ==> scong) (fun v=> s_par x v).
  Proof. constructor. eauto with brocs. Qed.
End ParScong.
Import ParScong.
...
Proof.
    exists (lcomm pt_s pt_c msg_quit).
    ...
    econstructor 2 with 
    (M1':=  (pt_s <-- P_s) ||| (pt_c <-- P_c_unf 30) ||| (pt_a <-- P_a 40))
    ...
    |||  (pt_a <-- P_a 40)).
    {
        ...
        assert(Hz3 : unfoldP ((pt_c <-- P_c_unf 30) ||| s_zero) (pt_c <-- P_c_unf 30)) by eauto with procs.
        eauto with procs.
    }
    {
      eauto with procs. 
    ...
    econstructor.
    constructor 1 with (l:=lcomm pt_s pt_c msg_quit).
    intros.